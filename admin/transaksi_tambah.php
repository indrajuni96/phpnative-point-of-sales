...
if (isset($_POST['submit'])) {
  ...
  if (isset($_SESSION['cart'])) { // cart sudah ada
    $item_array_id = array_column($_SESSION['cart'], "barang_id");

    if (in_array($_POST['barang_id'], $item_array_id)) {
      echo "<script>alert('Product is already added in the cart..!')</script>";

      foreach ($_SESSION['cart'] as $key => $value) {
        if ($value['barang_id'] == $_POST['barang_id']) {
          $_SESSION['cart'][$key]['jumlah'] = $value['jumlah'] + $_POST['jumlah'];
        }
      }
    } else {
